...
namespace OODBDemo.DBAccess
{
    class Checkinput
    {
        ...
        public int kiemtrangay(string ngay)
        {
            DateTime Test;
            bool t;
            t = DateTime.TryParse(ngay, out Test);
        ...
        public int kiemtradb_malop(string malop)
        {
           try
            {
                dbConnect.Open();
                Lop lopchk = (from Lop p in dbConnect.db
                                where p.Malop == malop
                                select p).SingleOrDefault();
                if (lopchk == null)
                    return 0;
        ...
        /*public int kiemtradb_manghanh(string manghanh)
        {
            ...
            {
                dbConnect.Open();
                Chuyennghanh nghanhchk = (from Chuyennghanh p in dbConnect.db
    ...
    /*    public int kiemtradb_manghanh_khoa(string manghanh,string makhoa)
        ...
        public int kiemtradb_makhoa(string makhoa)
        {
            ...
            {
                dbConnect.Open();
                Khoa khoachk = (from Khoa p in dbConnect.db
        ...
        public int kiemtra_sinhvien(string cmdd)
        {
            ...
            {
                dbConnect.Open();
                List<Sinhvien> svexist = (from Sinhvien p in dbConnect.db
                                where p.Cmnd == cmdd
                                select p).ToList();
